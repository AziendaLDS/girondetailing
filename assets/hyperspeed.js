...
const GIRON_RED = 0xe2181f;
const GIRON_RED_DEEP = 0xb9141a;
...
const DEFAULT_EFFECT_OPTIONS = {
  ...
  colors: {
    ...
    background: 0x0c0c0d,
    shoulderLines: GIRON_RED,
    brokenLines: GIRON_RED,
    leftCars: [GIRON_RED, GIRON_RED_DEEP, 0xff4a50],
    rightCars: [GIRON_RED, 0x7a1014, 0xf0a0a3],
    sticks: GIRON_RED
  }
};
...
class App {
  ...
  initPasses() {
    ...
    this.bloomPass = new EffectPass(
      this.camera,
      new BloomEffect({ luminanceThreshold: 0.2, luminanceSmoothing: 0, intensity: 1.4 })
    );
...
export function mountHyperspeed(container, effectOptions = {}) {
  ...
  options.distortion = turbulentDistortion;
...
function boot() {
  ...
  const container = document.getElementById('contactHyperspeed');
  if (!section || !container || reduced.matches) return;
  let app = null;
  const io = new IntersectionObserver(
    (entries) => {
      const visible = entries.some((entry) => entry.isIntersecting);
      if (visible && !app) app = mountHyperspeed(container);
      if (app) app.paused = !visible;
    },
