/**
 * Lightfall — vanilla port of React Bits Lightfall (ogl)
 * https://reactbits.dev
 */
...
const prepColors = (input) => {
  const base = (input && input.length ? input : ['#FF8A3D', '#C6631F', '#E8904A']).slice(0, MAX_COLORS);
...
export function mountLightfall(container, options = {}) {
  if (!container) return () => {};
  ...
  const {
    colors = ['#FF8A3D', '#C6631F', '#E8904A'],
    backgroundColor = '#C6631F',
    speed = 0.55,
    streakCount = 6,
    streakWidth = 1,
    streakLength = 1.1,
  ...
  // Listen on section so form doesn't block mouse glow
  const host = container.closest('.cta') || container;
  if (mouseInteraction) {
...
const el = document.getElementById('ctaLightfall');
if (el) {
  mountLightfall(el, {
    colors: ['#FF8A3D', '#C6631F', '#FFB070'],
    backgroundColor: '#C6631F',
    speed: 0.55,
    streakCount: 6,
    streakWidth: 1,
